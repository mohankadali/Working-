	...
	bool on)
{
	...
	if (on) {
printk(KERN_INFO "pdata->irq_gpio:%d\n",pdata->irq_gpio);
...
printk(KERN_INFO "pdata->hpd_irq_gpio:%d\n",pdata->hpd_irq_gpio);
...
printk(KERN_INFO "pdata->switch_gpio:%d\n",pdata->switch_gpio);
...
printk(KERN_INFO "mohan adv7533_probe\n");
	...
	if (!(pdata->disable_gpios)) {
printk(KERN_INFO "probe gpio config\n");
...
printk(KERN_INFO "pdata->video_mode = %d\n",pdata->video_mode);
...
printk(KERN_INFO "adv7533_probe pm_runtime_set_active\n");
	return 0;
...
static int adv7533_remove(struct i2c_client *client)
{
	...
	devm_kfree(&client->dev, pdata);
printk(KERN_INFO "adv7533_remove ret:%d\n",ret);
	return ret;
}
