...
static struct proc_dir_entry *spi_adc_proc;
int final_voltage = 0;
// our Driver Name
...
ssize_t spidev_read_proc(struct file *, char __user *, size_t, loff_t *);
...
//static ssize_t spidev_read_proc(struct file *fp, char __user*,ssize_t len, loff_t *off);
...
ssize_t spidev_read_proc(struct file *fp, char __user* ch,size_t len, loff_t *off)
{
				printk("proc entry heppened ..\n");
				...
				if( spi_sync(spi_prob_dev, &m) == 0) {
								...
								*p  = buff[1];
								...
								//res = copy_to_user((int *)arg, &final_Voltage, sizeof(int));
...
int my_map(struct file *filp, struct vm_area_struct *vma)
{
				printk("mmap function being calle...\n");
				int ret;
				...
				//init_memory( &i, kmalloc_area, LEN );
...
static int __init spi_dev_init(void)
{
				...
				struct proc_dir_entry *ret_struct;
				get_pid1 = current;
...
///////////////////////////////////////////////////////////////////////
				ret_struct = proc_create("spi_adc_proc", 0444, NULL,&fops);
				if(ret_struct == NULL ) {
								printk("proc entry is sucsess..\n");
								return 0;
				} else {
								printk("proc entry is sucsess....\n");
				}
...
static void spi_dev_exit (void)
{
				...
				cdev_del(my_cdev);
				unregister_chrdev_region(mydev, NR_OF_DEVS);
				remove_proc_entry("spi_adc_proc", NULL);
				return;
}
