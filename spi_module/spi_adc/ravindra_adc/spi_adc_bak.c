...
ssize_t spidev_read(struct file * fp, char __user * ch, size_t sz, loff_t * lf)
{
				...
				if( spi_sync(spi_prob_dev, &m) == 0) {
    		        ...
    		        res = copy_to_user(ch, &i, sizeof(int));
...
/* 
	 ...
	 once our driver load into kernal, using device tree entry ,kernal get to know that device is connected
